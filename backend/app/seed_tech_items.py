...
FUNCTION_INFO = {
    ...
    # 로컬 확장 (config_subtitle.yml 에 아직 없음)
    "validate_tilde_remains": (
        "물결(~) 잔존 검증",
        "노래·음향을 음표로 표기하는 방송사에서 물결이 정리되지 않고 남아 있으면 검출한다. "
        "전각 물결(～)·물결 대시(〜)·물결 연산자(∼)도 함께 본다(후처리는 ASCII ~ 만 다뤄 전각은 남는다). "
        "숫자 사이 물결은 범위 표현이라 정상으로 본다(1~2, 3 ~ 4)."),
