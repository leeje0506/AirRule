...
계정은 `AIRRULE_USERS` 환경변수(JSON 배열)로 넘긴다. 자세한 형식은
아래 [배포](#배포-vercel) 절 참고.

로컬에서 환경변수 없이 띄우면 `backend/app/accounts.py` 의 개발용 기본 계정
3개(admin / dev1 / sub1)가 뜨고 경고가 남는다. 배포 환경에서는 쓰이지 않는다.

역할(`role`)은 `admin` | `dev` | `subtitle` 이며, 읽기 전용 배포에서는
화면 표시 용도다.
